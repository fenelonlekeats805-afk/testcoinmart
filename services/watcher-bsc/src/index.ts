...
const pollMs = Number(process.env.BSC_WATCH_POLL_MS || 5000);
const confirmThreshold = Number(process.env.BSC_CONFIRM_THRESHOLD || 12);
const startupLookbackBlocks = Number(process.env.BSC_STARTUP_LOOKBACK_BLOCKS || 1200);
const scanStep = Math.max(1, Number(process.env.BSC_LOG_BLOCK_STEP || 9));
const addressChunkSize = Number(process.env.BSC_LOG_ADDRESS_CHUNK || 10);
const scanDelayMs = Number(process.env.BSC_LOG_REQUEST_DELAY_MS || 250);
...
async function loop() {
  while (true) {
    try {
      ...
      console.error('[watcher-bsc] scan failed', error);
    }
...
loop().catch((error) => {
  // eslint-disable-next-line no-console
  console.error('[watcher-bsc] fatal', error);
  process.exit(1);
});
